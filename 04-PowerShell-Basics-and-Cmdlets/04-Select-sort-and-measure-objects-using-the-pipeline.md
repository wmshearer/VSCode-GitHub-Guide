...
Measure-Object (alias: measure) counts and calculates numeric data from pipeline objects.
...
🧠 Tip:
You can shorten parameters like -Minimum → -Min and -Maximum → -Max, but not -Average (you can use -Ave instead).

Step 3. Select a Set of Objects in the Pipeline

Select-Object (alias: select) can limit or customize command output.
...
🧩 Note: Some column headers displayed in default tables aren’t the actual property names (e.g., CPU(s) is really CPU).
...
Default behavior of Sort-Object:
➜ Case-insensitive sort in ascending order.

Cmdlet used to choose displayed properties:
➜ Select-Object